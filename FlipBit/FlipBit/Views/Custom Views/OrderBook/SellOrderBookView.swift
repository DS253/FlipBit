...
class SellOrderBookView: View {
    ...
    override func setup() {
        super.setup()
        NotificationCenter.default.addObserver(self, selector: #selector(updateBookRows(notification:)), name: .sellBookObserverUpdate, object: nil)
    }
    ...
    @objc func updateBookRows(notification: Notification) {
        guard let book = bookObserver.sellBook else { return }
        if book.count >= 1 {
